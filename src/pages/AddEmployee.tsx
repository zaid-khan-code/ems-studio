...
export default function AddEmployee() {
  ...
  useEffect(() => {
    if (isEditMode && editId) {
      ...
      if (emp) {
        ...
        setContact1(emp.contact1 || "");
        setContact2(emp.contact2 || "");
        setIce1(emp.emergency1 || "");
        setIce2(emp.emergency2 || "");
        setPermAddress(emp.permanentAddress || "");
        setPostAddress(emp.postalAddress || "");
        setBankName(emp.bankName || "");
        setBankAccount(emp.bankAccount || "");
        setPaymentMode(emp.paymentMode || "Online Transfer");
        setDept(emp.department || departments[0]);
        setDesig(emp.designation || designations[0]);
        setEmpType(emp.employmentType || employmentTypes[0]);
        setJobStat(emp.jobStatus || jobStatuses[0]);
        setWMode(emp.workMode || workModes[0]);
        setWLoc(emp.workLocation || workLocations[0]);
        setRm(emp.reportingManager || reportingManagers[0]);
        setShift(emp.shift || shifts[0]?.name);
        setDoj(emp.dateOfJoining || "");
        setDoe(emp.dateOfExit || "");
        setCommissionEligible(emp.commissionEligible || false);
        setSalBasic(emp.salary?.basic || 0);
        setSalHouse(emp.salary?.houseRent || 0);
        setSalMedical(emp.salary?.medical || 0);
        setSalConveyance(emp.salary?.conveyance || 0);
        setSalCommission(emp.salary?.commission || 0);
        setBloodGroup(emp.bloodGroup || "");
        setAllergies(emp.allergies || "");
        setChronic(emp.chronicConditions || "");
        setMedications(emp.medications || "");
      }
    } else {
  ...
  const deptDesignations = designationsByDept[dept] || designations;

  // Update designation when department changes
  useEffect(() => {
    const available = designationsByDept[dept] || designations;
    if (!available.includes(desig)) {
      setDesig(available[0] || "");
    }
  }, [dept, designationsByDept, designations, desig]);
  ...
  const totalSalary =
    ...
    salMedical +
    salConveyance +
    (commissionEligible ? salCommission : 0);
  ...
  const handleSave = () => {
    ...
    setTimeout(() => {
      const empData = {
        id: isEditMode ? editId! : employeeId,
        name: fullName,
        fatherName,
        dob,
        cnic,
        ...
        postalAddress: sameAddress ? permAddress : postAddress,
        bankName,
        bankAccount,
        paymentMode,
        bloodGroup,
        ...
        salary: {
          ...
          houseRent: salHouse,
          medical: salMedical,
          conveyance: salConveyance,
          commission: commissionEligible ? salCommission : 0,
        },
      };
        ...
        return (
          <div>
              ...
              <div className="form-group">
                ...
                <div style={{ position: "relative" }}>
                  <input
                    ...
                    value={employeeId}
                    disabled
                    style={{ background: "var(--steell)", paddingRight: 32 }}
                  />
                  <Lock
                    size={12}
                    style={{
            ...
            <div className="form-row-3">
              <div className="form-group">
                <label className="form-label">
                  CNIC <span style={{ color: "var(--red)" }}>*</span>
                </label>
                <input
                  className="input mono"
                  ...
                  style={errors.cnic ? { borderColor: "var(--red)" } : {}}
                />
                {errors.cnic && (
                  <div
                    style={{ color: "var(--red)", fontSize: 11, marginTop: 4 }}
            ...
            <div className="form-row">
              <div className="form-group">
                ...
                <input
                  className="input"
                  value={ice1}
                  onChange={(e) => setIce1(e.target.value)}
                />
              </div>
              <div className="form-group">
                ...
                <input
                  className="input"
                  value={ice2}
                  onChange={(e) => setIce2(e.target.value)}
                />
              </div>
            </div>
            <div className="form-group">
          ...
          <div>
            <div className="form-row-3">
              ...
              <div className="form-group">
                ...
                >
                  {deptDesignations.map((d: string) => (
                    <option key={d}>{d}</option>
                  ))}
                </select>
                <div style={{ fontSize: 10, color: "var(--t3)", marginTop: 4 }}>
                  Showing designations for {dept}
                </div>
              </div>
              <div className="form-group">
            ...
            <div className="form-row-3">
              <div className="form-group">
                ...
                >
                  {reportingManagers.map((d: string) => (
                    <option key={d}>{d}</option>
                  ))}
                </select>
              </div>
                ...
                <div style={{ display: "flex", gap: 12, marginTop: 6 }}>
                  <label style={{ fontSize: 12.5, cursor: "pointer" }}>
                    <input
                      ...
                      onChange={() => setCommissionEligible(true)}
                    />{" "}
                    Yes
                  </label>
                  <label style={{ fontSize: 12.5, cursor: "pointer" }}>
                    <input
                      type="radio"
                      checked={!commissionEligible}
                      onChange={() => setCommissionEligible(false)}
                    />{" "}
                    No
                  </label>
                </div>
        ...
        return (
          <div>
            <div style={{ fontSize: 11, color: "var(--t3)", marginBottom: 12 }}>
              Salary components are configurable from Settings → Payroll
              Components
            </div>
                    ...
                    <tr>
                      ...
                      <td>
                        <input
                          ...
                          type="number"
                          value={salCommission || ""}
                          onChange={(e) => setSalCommission(+e.target.value)}
                          style={{ width: 160 }}
                        />
                      </td>
                      <td>
            ...
            >
              ...
              <span
                className="mono"
                style={{ fontSize: 18, fontWeight: 800, color: "var(--p)" }}
              >
            ...
            <div style={{ marginTop: 12 }}>
              <DecisionBanner>
                DECISION NEEDED — Salary Components: Same for all or per person?
                Confirm in meeting.
              </DecisionBanner>
            </div>
          </div>
      ...
      case 5:
        return (
          <div>
            ...
            <div style={{ marginTop: 12 }}>
              <DecisionBanner>
                DECISION NEEDED — Can employees edit their own medical info?
                Confirm in meeting.
              </DecisionBanner>
            </div>
          </div>
      ...
      case 6:
        return (
          <div>
            ...
            <div style={{ marginTop: 12 }}>
              <DecisionBanner>
                DECISION NEEDED — Which attachments are mandatory? Confirm in
                meeting.
              </DecisionBanner>
            </div>
          </div>
        ...
        return (
          <div>
            ...
            >
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  background:
                    accountMethod === "A" ? "var(--p)" : "transparent",
                  color: accountMethod === "A" ? "#fff" : "var(--t2)",
                }}
              >
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  background:
                    accountMethod === "B" ? "var(--p)" : "transparent",
                  color: accountMethod === "B" ? "#fff" : "var(--t2)",
                }}
              >
                Send Invite Link
              </button>
            ...
            {fullName && (
              ...
              >
                <strong>{fullName}</strong> · {dept} · {desig} · {shift}
                <br />
            ...
            <div style={{ marginTop: 12 }}>
              <DecisionBanner>
                DECISION NEEDED — Account creation method: A or B? Confirm in
                meeting.
              </DecisionBanner>
            </div>
          </div>
              ...
              >
                <div
                  style={{
                    ...
                    border: i > step ? "1.5px solid var(--br)" : "none",
                  }}
                >
              ...
              {i < STEPS.length - 1 && (
                <div
                  style={{
                    flex: 1,
                    height: 2,
                    background: i < step ? "var(--green)" : "var(--br)",
                    margin: "0 -4px",
                    marginBottom: 16,
                  }}
                />
