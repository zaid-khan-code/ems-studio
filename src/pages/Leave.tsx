...
export default function Leave() {
  ...
  const filtered = tab === 'all' ? data : data.filter((l: any) => l.status.toLowerCase() === tab);
  ...
  function openEdit(row: any) {
    setEditType(row.leaveType); setEditFrom(row.from); setEditTo(row.to); setEditReason(row.reason);
    setEditModal(row);
  }
  ...
  function saveEdit() {
    ...
    setSaving(true);
    setTimeout(() => {
      setData(prev => prev.map((l: any) => l.id === editModal.id ? {
        ...l, leaveType: editType, from: editFrom, to: editTo,
        days: calcDays(editFrom, editTo), reason: editReason
      } : l));
      setSaving(false); setEditModal(null); showToast('Leave request updated');
    }, 600);
  }
  ...
  function confirmEarly() {
    ...
    setTimeout(() => {
      ...
      setData(prev => prev.map((l: any) => l.id === earlyModal.id ? { ...l, to: earlyDate, days: actualDays } : l));
  ...
  function submitNew() {
    ...
    setTimeout(() => {
      ...
      const newRow = {
        ...
        empId: newEmp, empName: emp?.name || '', leaveType: newType,
        from: newFrom, to: newTo, days: newDays, reason: newReason,
        appliedOn: new Date().toISOString().split('T')[0], status: 'Pending',
      };
      setData(prev => [newRow, ...prev]);
      setSaving(false); setNewModal(false);
  ...
  return (
    <div>
      ...
      <div className="card">
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: 40, color: 'var(--t3)' }}>
            <CalendarDays size={32} style={{ margin: '0 auto 8px', opacity: .4 }} /><div style={{ fontSize: 13 }}>No leave requests found</div>
          </div>
        ) : (
          <table>
            ...
            <tbody>
              {filtered.map((l: any) => (
                <tr key={l.id}>
                  ...
                  <td>{l.leaveType}</td><td className="mono">{l.from}</td><td className="mono">{l.to}</td>
      ...
      <Modal open={newModal} onClose={() => setNewModal(false)} title="New Leave Request" footer={
        ...
        <div className="form-group"><label className="form-label">Leave Type</label>
          <select className="input select-input" value={newType} onChange={e => setNewType(e.target.value)}>
            {leaveTypes.filter((t: any) => t.active).map((t: any) => <option key={t.code} value={t.name.replace(' Leave', '')}>{t.name}</option>)}
          </select>
        </div>
      ...
      <Modal open={!!earlyModal} onClose={() => setEarlyModal(null)} title="Mark Early Return" footer={
        ...
        <div className="form-group"><label className="form-label">Employee returned on:</label>
          <input className="input" type="date" value={earlyDate} onChange={e => setEarlyDate(e.target.value)} min={earlyModal?.from} max={earlyModal?.to} />
        </div>
        {earlyDate && (
