...
export default function Leave() {
  ...
  const filtered =
    tab === "all"
      ? data
      : data.filter((l: any) => l.status.toLowerCase() === tab);

  function handleApprove(id: string) {
    setData((prev) =>
      prev.map((l: any) => (l.id === id ? { ...l, status: "Approved" } : l)),
    );
    showToast("Leave approved");
  }
  function handleReject(id: string) {
    setData((prev) =>
      prev.map((l: any) => (l.id === id ? { ...l, status: "Rejected" } : l)),
    );
  ...
  function confirmEarly() {
    ...
    setTimeout(() => {
      ...
      setData((prev) =>
        prev.map((l: any) =>
          l.id === earlyModal.id
            ? { ...l, to: earlyDate, days: actualDays }
            : l,
        ),
      );
      setSaving(false);
  ...
  function submitNew() {
    if (!newEmp || !newFrom || !newTo || !newReason) {
  ...
  const earlyOrigDays = earlyModal
    ? calcDays(earlyModal.from, earlyModal.to)
    : 0;
  const earlyActual =
    earlyModal && earlyDate
      ? calcDays(earlyModal.from, earlyDate)
      : earlyOrigDays;
    ...
    <div>
      <div className="pg-head">
        <div>
          ...
          <div className="pg-sub">Manage leave requests and approvals</div>
        </div>
        ...
        <div className="form-group">
          ...
          >
            {leaveTypes
              ...
              .map((t: any) => (
                <option key={t.code} value={t.name.replace(" Leave", "")}>
                  {t.name}
                </option>
              ))}
          </select>
      ...
      >
        <DecisionBanner>
          DECISION NEEDED — HR Edit Permissions: Which fields can HR edit?
      ...
      <Modal
        open={!!earlyModal}
        onClose={() => setEarlyModal(null)}
        title="Mark Early Return"
        footer={
          <>
            <button
              className="btn btn-secondary"
              onClick={() => setEarlyModal(null)}
            >
              Cancel
            </button>
            <button
              ...
              disabled={saving || !earlyDate}
            >
              {saving ? "Saving..." : "Confirm Early Return"}
            </button>
          </>
        }
      >
        ...
        >
          <div style={{ fontWeight: 600, marginBottom: 2 }}>
            {earlyModal?.empName} — {earlyModal?.leaveType} Leave
          </div>
          <div className="mono" style={{ fontSize: 11, color: "var(--t3)" }}>
            Original: {earlyModal?.from} to {earlyModal?.to} ({earlyOrigDays}{" "}
            days)
          </div>
        </div>
        <div className="form-group">
        ...
        {earlyDate && (
          ...
          >
            <div>
              Days Actually Taken:{" "}
              <strong className="mono">{earlyActual}</strong>
            </div>
            <div>
              Days to Restore:{" "}
              <strong className="mono" style={{ color: "var(--green)" }}>
                {earlyRestore > 0 ? earlyRestore : 0}
              </strong>
            </div>
          </div>
        )}
        <div style={{ fontSize: 11, color: "var(--t3)", marginTop: 8 }}>
          The original leave approval is preserved. The early return date is
          recorded separately.
        </div>
      </Modal>
    </div>
