...
import { departments, designations, workModes, workLocations, employmentTypes, jobStatuses, shifts, leaveTypes, leavePolicies, payrollComponents, penaltiesConfig } from '../../data/dummyData';
...
export function PenaltiesConfigPage() { return <SettingsPage title="Penalties Config" columns={['Name', 'Category', 'Default Fine', 'Active']} data={penaltiesConfig.map(p => ({ name: p.name, category: p.category, fine: `Rs ${p.defaultFine.toLocaleString()}`, active: p.active }))} />; }
