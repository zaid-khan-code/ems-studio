...
export default function Attendance() {
  ...
  // Handle URL query parameter for highlighting unmarked employees
  useEffect(() => {
    const highlightParam = searchParams.get("highlight");
    if (highlightParam === "unmarked") {
  ...
  const initRows = (): AttRow[] =>
    employees.map((e: any) => {
      ...
      const existing =
        e.id === "EMP001"
          ? { checkIn: "09:02", checkOut: "18:05", status: "Present" }
          : e.id === "EMP002"
            ? { checkIn: "08:55", checkOut: "18:00", status: "Present" }
            : e.id === "EMP003"
              ? { checkIn: "09:22", checkOut: "18:10", status: "Late" }
              : e.id === "EMP004"
                ? { checkIn: "-", checkOut: "-", status: "On Leave" }
  ...
  const updateRow = (
    ...
    setRows((prev) => {
      ...
      if (field === "checkIn") {
        ...
        next[realIdx].lateBy = calcLateBy(
          next[realIdx].expectedIn,
          value as string,
          s?.lateAfter || 15,
        );
        next[realIdx].status = autoStatus(
          next[realIdx].expectedIn,
          value as string,
          s?.lateAfter || 15,
          next[realIdx].status,
  ...
  const saveAll = () => {
    localStorage.setItem(
      "ems_attendanceSheet_" + selectedDate,
      JSON.stringify(rows),
    );
    ...
    <div>
      <div className="pg-head">
        <div>
          ...
          <div className="pg-sub">Track and manage employee attendance</div>
        </div>
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn btn-ghost" onClick={handleExportClick}>
            <Download size={13} /> Export Sheet PDF
          </button>
          <button className="btn btn-ghost" onClick={handleExportClick}>
      ...
      <DecisionBanner>
        DECISION NEEDED — Attendance Verification Method: Manual / Self-mark /
        Biometric? Confirm in meeting.
      </DecisionBanner>
      <DecisionBanner>
        ...
        <br />
        If yes: should it export the current day view, the monthly report, or
        both?
        <br />
        ...
        <div>
          ...
          >
            {Object.entries(summary).map(([k, v]) => (
              <span
                key={k}
                className={`pill ${k === "Present" ? "pill-green" : k === "Late" ? "pill-amber" : k === "Absent" ? "pill-red" : k === "On Leave" ? "pill-blue" : "pill-steel"}`}
              >
                {k}: {v}
              </span>
            ))}
          ...
          {summary.Unmarked > 0 && (
            ...
            >
              ⚠ {summary.Unmarked} employee(s) not yet marked
            </div>
          )}
              ...
              <table>
                ...
                <tbody>
                  {filteredRows.map((r, i) => {
                    const isUnmarked = !r.acknowledged || r.status === "Absent";
                    const shouldHighlight = highlightUnmarked && isUnmarked;
                    return (
                      <tr
                        key={r.empId}
                        style={{
                      ...
                      >
                        ...
                        <td style={{ textAlign: "center" }}>
                          <input
                            ...
                            onChange={(e) =>
                              updateRow(i, "acknowledged", e.target.checked)
                            }
                          />
                        </td>
                      </tr>
          ...
          >
            <span style={{ fontSize: 12, color: "var(--t2)" }}>
              Total: {filteredRows.length} | Present: {summary.Present} | Late:{" "}
      ...
      {tab === "monthly" && (
        <div className="card">
          <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
            <select className="input select-input" style={{ width: 120 }}>
              <option>March</option>
